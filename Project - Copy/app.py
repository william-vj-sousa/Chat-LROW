...
class Classroom(db.Model):
    ...
    assignments = db.relationship('Assignment', backref='classroom', lazy=True, order_by='Assignment.deadline.asc()')
...
class Submission(db.Model):
    ...
    status = db.Column(db.String(20), nullable=False, default='pending')  # pending, in_progress, submitted
...
def classroom_detail(classroom_id):
    ...
    if current_user.role == 'teacher':
        ...
        for assignment in classroom.assignments:
            submitted = Submission.query.filter_by(assignment_id=assignment.id, status='submitted').count()
            in_progress = Submission.query.filter_by(assignment_id=assignment.id, status='in_progress').count()
...
@app.route('/classroom/<int:classroom_id>/prompt')
@login_required
...
@app.route('/classroom/<int:classroom_id>/assignment/create', methods=['POST'])
@login_required
def create_assignment(classroom_id):
    ...
    if not instructions:
        return jsonify({"error": "Les instructions sont requises."}), 400
    if not deadline_str:
        return jsonify({"error": "La date limite est requise."}), 400
    ...
    if deadline < datetime.utcnow():
        return jsonify({"error": "La date limite doit être dans le futur."}), 400
    ...
    return jsonify({
        ...
        "deadline": assignment.deadline.strftime('%d/%m/%Y à %H:%M'),
...
@app.route('/assignment/<int:assignment_id>/work')
@login_required
def assignment_work(assignment_id):
    if current_user.role != 'student':
        flash("Accès refusé.")
        return redirect(url_for('dashboard'))
    ...
    if submission is None:
        conversation = Conversation(
            ...
            title=f"Devoir: {assignment.title}"
        )
        db.session.add(conversation)
        db.session.flush()
        submission = Submission(
